...
const AlertDashboardComponent = ({ onBack, token }) => {
    ...
    const fetchData = async () => {
        try {
            ...
            if (hRes.ok) {
                ...
                // FIX: Filter out logs that were cleared by the user
    ...
    const handleConfirmClear = async () => {
        // 1. Add current history IDs to the ignored set so they don't come back
        history.forEach(item => {
            ignoredHistoryIdsRef.current.add(item.id);
        });

        // 2. Attempt to clear via backend (Optional/Best Effort)
        try {
            const res = await fetch("http://localhost:8000/alerts/history", {
                method: "DELETE",
                headers: { 'Authorization': `Bearer ${token}` }
            });
            // If backend doesn't support delete, we rely on the frontend filter
        } catch (e) {
            console.log("Backend clear skipped or failed");
        }
        ...
        // 4. Feedback
        if(window.showToast) window.showToast("Incident Log Cleared", "success");
    };
        ...
        return (
            <main className="alert-content-area">
                ...
                <ConfirmModal
                    ...
                    message="Are you sure you want to permanently clear the incident log history? This action cannot be undone."
                />
            </main>
        );
    };
